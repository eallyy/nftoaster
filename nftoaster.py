...
nftoaster = argparse.ArgumentParser(prog='nftoaster', usage='nftoaster.py [options]')

nftoaster.add_argument('-n', '--amount', help="Amount of NFT to generate")
nftoaster.add_argument('-c', '--config', help="Path of configuration file")

args = nftoaster.parse_args()

if args.amount and args.config:
  if pathExists(args.config):
    generate_unique_images(int(args.amount), loadJSON(args.config))
  else:
    print('NFToaster: Configuration file specified doesn\'t exist.\n')

else:
  print('NFToaster: Missing a mandatory option (-n or -c). Use -h to show the help menu.\n')
